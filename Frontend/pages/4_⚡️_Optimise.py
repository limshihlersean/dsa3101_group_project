...
#Get the data generated by the backend to post it to the frontend 
# Send a GET request to fetch optimized values from the backend
response = requests.get('https://example.com/api/get_optimized_values')

# Check if the request was successful
if response.status_code == 200:
    # Extract the optimized values from the response
    optimized_values = response.json()
    
    # Display the optimized values
    st.write("Optimized Values:")
    st.write(optimized_values)
else:
    st.error(f"Error: {response.status_code}")
